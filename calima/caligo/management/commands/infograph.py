...
class InfoGraphic():
    ...
    def generate_by_year(self, stations, daily_reports, year):
        ...
        for station in stations:
            ...
            station_pos[station.code] = pos_y
        ...
        for station in stations:
            ...
            p_y = margin_y + station_pos[station.code] + int((PIX_SIZE - font_size[1]) / 2)
    ...
    def generate_by_average(self, stations, year_min, year_max):
        """ Generates an infographic image showing temperature """
        ...
        for station in stations:
            ...
            station_pos[station.code] = pos_y
            pos_y = pos_y + PIX_SIZE
        margin_x = margin_x + PIX_SIZE
        # Create image
        margin_x = margin_x + MARGIN_FONT
        size_x = margin_x + (years_num + 1) * PIX_SIZE
        size_y = (stations_num + 3) * PIX_SIZE + margin_y
        im = Image.new("RGB", (size_x, size_y), "White")
        draw = ImageDraw.Draw(im)
        draw.setfont(font)
        # Draw title
        title = "Mapa de temperaturas anuales promedio - Datos AEMET - calima.linotipo.es"
        font_size = font.getsize(title)
        p_x = (size_x - font_size[0]) / 2
        p_y = 0
        draw.text((p_x, p_y), title, fill="Black")
        ...
        for station in stations:
            ...
            p_x = PIX_SIZE
            # draw.rectangle([p_x, p_y, margin_x - 1, p_y + PIX_SIZE - 1], fill=color)
            draw.rectangle([p_x, p_y, size_x - PIX_SIZE, p_y + PIX_SIZE - 1], fill=color)
            # Draw text
            font_size = font.getsize(station_title(station))
            p_x = margin_x - font_size[0] - int(MARGIN_FONT / 2)
            p_y = margin_y + station_pos[station.code] + int((PIX_SIZE - font_size[1]) / 2)
       ...
       # Draw temperature palette
        ...
        for station in stations:
            ...
            for year in range(year_min, year_max + 1):
                ...
                if (not avg):
                    print 'ERROR: %s %i %d (%s)' % (station.name, year, count, connection.queries[-1])
                    continue
                average = float(avg)
                p_x = margin_x + (year - year_min) * PIX_SIZE
                p_y = margin_y + station_pos[station.code]
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
            make_option('--year', '-y', action='store_true', dest='year'),
            make_option('--station', '-s', action='store_true', dest='station'),
            make_option('--average', '-a', action='store_true', dest='average'),
        )
    ...
    def handle(self, *args, **options):
        ...
        elif (options.get('average')):
            image = args[0]
            graph = InfoGraphic(image)
            graph.image_by_average()
